...
namespace MCO.DiscordBot
{
    public class StatsService
    {
        ...
        public async Task<byte[]> GetStatsImageAsync()
        {
            ...
            var largeFont = new Font("Roboto Medium", 54);
            var mediumFont = new Font("Roboto Medium", 25);
            var smallFont = new Font("Roboto", 16);
            ...
            using(var bg = Bitmap.FromFile("MCO_bg.png"))
            {
                using(var memStr = new MemoryStream())
                {
                    ...
                    gr.DrawString(FormattableString.Invariant($"MCO Stats on {DateTime.UtcNow:yyyy-MM-dd HH:mm:ss} UTC"), smallFont, grayBrush, 380, 22);
                    ...
                    gr.DrawString(usdPercentage, mediumFont, mcoDollarPercentage >= 0m ? greenBrush : redBrush, bg.Width - usdPercentageSize.Width - left + 3, top + 57);
                    var usdPrice = FormattableString.Invariant($"${mcoDollarPrice:0.00}");
                    var usdPriceSize = gr.MeasureString(usdPrice, largeFont);
                    gr.DrawString(usdPrice, largeFont, mcoBluebrush, bg.Width - usdPriceSize.Width - left - usdPercentageSize.Width + 20, top + 20);
                    ...
                    gr.DrawString(btcPrice, mediumFont, blackBrush, left, top += 20);
                    ...
                    gr.DrawString(ethPrice, mediumFont, blackBrush, left, top += 30);
                    ...
                    gr.DrawString("24H Volume (Binance)", smallFont, grayBrush, left + leftFix, top += 40);
                    gr.DrawString(FormattableString.Invariant($"{btcMcoPrice.Data.QuoteVolume:0.00} BTC / {btcMcoPrice.Data.Volume:#,##0.00} MCO"), mediumFont, blackBrush, left, top += 22);
                    gr.DrawString(FormattableString.Invariant($"{ethMcoPrice.Data.QuoteVolume:0.00} ETH / {ethMcoPrice.Data.Volume:#,##0.00} MCO"), mediumFont, blackBrush, left, top += 30);

                    gr.DrawString("Reservations", smallFont, grayBrush, left + leftFix, top += 40);
                    gr.DrawString(FormattableString.Invariant($"Total: {reservations.Total:#,###}"), mediumFont, blackBrush, left, top += 20);
                    gr.DrawString(FormattableString.Invariant($"Today: {reservations.Today:#,##0}"), mediumFont, blackBrush, left, top += 30);

                    gr.DrawString("BTC Price", smallFont, grayBrush, left + leftFix, top += 40);
                    gr.DrawString(FormattableString.Invariant($"${btcUsdtPrice.Data.LastPrice:#,###,###.00}"), mediumFont, blackBrush, left, top += 20);
